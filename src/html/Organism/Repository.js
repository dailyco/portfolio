...
import Topics from "../Molecule/Topics";

function Repository({ data }) {
  const { name, star, fork, github, demo, desc, topics } = data;
  ...
  return (
    <div className="project neumorphism-card" data-aos="zoom-in-down">
      <div className="name-link-wrapper">
        <h3>{name}</h3>
        <div className="links">
          <a href={github + "/stargazers"} className="project-stars-forks">
          ...
          <a href={github + "/network/members"} className="project-stars-forks">
            <svg viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true">
              <path
                fill-rule="evenodd"
      ...
      <Url url={github} label="GitHub"></Url>
      {demo && <Url url={demo} label="Demo"></Url>}
